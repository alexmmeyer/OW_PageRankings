...
def update(name1, name2, gender_choice):
    ...
    for i in range(len(df)):
        if df['winner'][i] == name1:
            marker_color = 'blue'
        elif df['winner'][i] == name2:
            marker_color = 'green'
        else:
            marker_color = 'red'
        trace = go.Scatter(
            ...
            y=[0],
            mode='markers',
            marker={'color': marker_color, 'size': 20, 'line': {'width': 0.5, 'color': 'black'}, 'opacity': 0.5}
        )
...
update("Gregorio Paltrinieri", "Florian Wellbrock", "men")